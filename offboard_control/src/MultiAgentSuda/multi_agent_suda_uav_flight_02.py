...
def next_game_and_agent_state(automaton, game_state, target_state, iset, OR_state):
    ...
    if is_vis:
        ...
        for k in automaton[game_state]['Successors']:
            if automaton[k]['State']['st'] > OR_state - 1:
                new_game_state = k
                break
...
if __name__ == "__main__":
    ...
    OR_state = rospy.get_param('~outside_region_state')
    z_nom = 1.5
    ...
    while not rospy.is_shutdown():
        ...
        try:
            ...
            if segway_state not in allowed_states:
                segway_state = OR_state
            game_state, quad_state = next_game_and_agent_state(automaton, game_state, segway_state, iset, OR_state)
